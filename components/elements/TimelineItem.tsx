...
const TimelineItem: FC<{ timeline: JobExperience }> = ({
  timeline: { title, meta, text, year, tags },
}) => {
  return (
    <div className="timeline relative block items-start rounded-full border-white border-opacity-20 first:mt-0 md:flex">
      <span className="timeline-year absolute left-12 top-0 mr-12 mt-7 min-w-[115px] rounded-full bg-primary bg-opacity-10 p-1 text-center text-sm leading-none md:relative md:left-0">
        {year}
      </span>
      <div className="timeline-right relative border-l-4 border-white border-opacity-20 pl-12 pt-16 md:pt-6">
        ...
        <div className="flex flex-row gap-2">
          {tags?.map((tag) => (
            <span className="rounded bg-primary bg-opacity-10 px-1 text-xs">
              {tag}
            </span>
          ))}
        </div>
